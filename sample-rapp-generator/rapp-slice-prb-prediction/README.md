...
if ran_client.ran_nssmf_address:
    ...
    # Modify PRB allocation based on predictions
    if subnet_info:
        current_prb = subnet_info["attributes"]["sliceProfileList"][0]["ransliceSubnetProfile"]["RRU.PrbDl"]
        predicted_usage = model.predict_prb_usage("slice-subnet-id")
        
        if predicted_usage > 80:  # High usage threshold
            new_prb = min(current_prb + 10, 100)
            response = ran_client.modify_network_slice_subnet("slice-subnet-id", new_prb)
            if response and response.status_code == 200:
                print(f"Increased PRB allocation to {new_prb}")
        elif predicted_usage < 30:  # Low usage threshold
            new_prb = max(current_prb - 5, 10)
            response = ran_client.modify_network_slice_subnet("slice-subnet-id", new_prb)
            if response and response.status_code == 200:
                print(f"Decreased PRB allocation to {new_prb}")
