...
kubectl exec -it "$KAFKA_POD_NAME" -n "$KAFKA_NAMESPACE" -- /opt/kafka/bin/kafka-console-consumer.sh \
  ...
  --consumer-property "sasl.jaas.config=org.apache.kafka.common.security.scram.ScramLoginModule required username=\"topology-ingestion-ku\" password=\"WxWdXVYSs3BWJNFJ9DWmtuj62SKxvezM\";" \
  --group "$GROUP_ID" \
  --from-beginning \
  --timeout-ms 60000 \
